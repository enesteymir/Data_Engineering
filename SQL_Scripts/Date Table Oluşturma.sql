...
(
  SELECT
    ...
    LastDateOfYear,
	BeforeToday = CASE WHEN [Date] < GETDATE()-1 THEN 1 ELSE 0 END 
...
set @thisyear = (select [Year] from #DAYTABLE where YearMonthDay = convert(char(8),getdate()-1,112))

declare @lastyear as char(4)
set @lastyear = (select [Year] from #DAYTABLE where YearMonthDay = convert(char(8),getdate()-366,112))

declare @ThisMonth as char(4)
set @ThisMonth = (select MonthNumber  from #DAYTABLE where YearMonthDay = convert(char(8),getdate()-1,112))

declare @LastMonthYear as char(4)
set @LastMonthYear = (select [Year] from #DAYTABLE where YearMonthDay = convert(char(8),getdate()-30,112))

declare @LastWeek as char(8)
set @LastWeek = (select WeekNumberOfYear from #DAYTABLE  where YearMonthDay = convert(char(8),getdate()-7,112))

declare @ThisWeek as char(8)
set @ThisWeek = (select WeekNumberOfYear from #DAYTABLE  where YearMonthDay = convert(char(8),getdate(),112))

declare @LastDay as char(8)
set @LastDay = (select YearMonthDay from #DAYTABLE  where YearMonthDay = convert(char(8),getdate()-1,112) )

declare @Today as char(8)
set @Today = (select YearMonthDay from #DAYTABLE where YearMonthDay = convert(char(8),getdate(),112) )

declare @LastDayYTD as char(8)
set @LastDayYTD = (select YearMonthDay from #DAYTABLE  where YearMonthDay = convert(char(8),getdate()-366,112) )

declare @YesterdayWeekDay as char(3)
set @YesterdayWeekDay =(select DayNumberOfWeek from #DAYTABLE  where YearMonthDay = convert(char(8),getdate()-1,112) )
...
CASE WHEN [Year]=@lastyear AND MonthNumber=@ThisMonth THEN 1 ELSE 0 END AS LastYearThisMonth,
CASE WHEN [Year]=@lastyear AND MonthNumber=@ThisMonth AND DayNumberOfMonth <= DATEPART(DAY,GETDATE()-1) THEN 1 ELSE 0 END AS LastYearThisMonthMTD,
...
CASE WHEN [Year]=@LastMonthYear AND MonthNumber= (CASE WHEN @ThisMonth=1 THEN 12 ELSE @ThisMonth-1 END) AND DayNumberOfMonth <= DATEPART(DAY,GETDATE()-1) THEN 1 ELSE 0 END AS LastMonthMTD,
...
SELECT * FROM DimDate
WHERE Year BETWEEN 2020 and YEAR(GETDATE())
ORDER BY 1 DESC